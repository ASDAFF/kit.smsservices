...
class Events {
	...
	public static function getList(){
		...
		$events = array(
			"MSMS_NEWORDER" => array(
				"BX_EVENT" => array(
					array('sale','OnSaleOrderSaved','kit.smsservices','\Kit\Smsservices\Events','OnSaleOrderEntitySaved','new'),
				),
				"FIELD" => array(
					"HTML" => array('\Kit\Smsservices\Fields','newOrderHtml'),
					"BEFORE_SAVE" => array('\Kit\Smsservices\Fields','newOrderSave')
				),
				"NAME" => Loc::getMessage("KIT_SMSSERVICES_EVENTCODE_MSMS_NEWORDER")
			),
			"MSMS_STATUSUPDATE" => array(
				...
				"FIELD" => array(
					"HTML" => array('\Kit\Smsservices\Fields','statusOrderHtml'),
					"BEFORE_SAVE" => array('\Kit\Smsservices\Fields','statusOrderSave')
				),
				"NAME" => Loc::getMessage("KIT_SMSSERVICES_EVENTCODE_MSMS_STATUSUPDATE")
			),
			"MSMS_PAYED" => array(
				"BX_EVENT" => array(
					array('sale','OnSaleOrderSaved','kit.smsservices','\Kit\Smsservices\Events','OnSaleOrderEntitySaved','new'),
				...
				"FIELD" => array(
					"HTML" => array('\Kit\Smsservices\Fields','payedOrderHtml'),
					"BEFORE_SAVE" => array('\Kit\Smsservices\Fields','payedOrderSave')
				),
				"NAME" => Loc::getMessage("KIT_SMSSERVICES_EVENTCODE_MSMS_PAYED")
			),/*
			"MSMS_RESET_PASSWORD" => array(
				"BX_EVENT" => false,
				"FIELD" => array(
					"HTML" => array('\Kit\Smsservices\Fields','resetPasswordHtml'),
					"BEFORE_SAVE" => array('\Kit\Smsservices\Fields','newOrderSave')
				),
				"NAME" => Loc::getMessage("KIT_SMSSERVICES_EVENTCODE_MSMS_RESET_PASSWORD")
			),*/
			/*"MSMS_BXEVENT" => array(
				"BX_EVENT" => array(
					array('main','OnBeforeEventSend','kit.smsservices','\Kit\Smsservices\Events','OnBeforeEventSend','old'),
				),
				"FIELD" => array(
					"HTML" => array('\Kit\Smsservices\Fields','eventSendHtml'),
					"BEFORE_SAVE" => array('\Kit\Smsservices\Fields','eventSendSave')
				),
				"NAME" => Loc::getMessage("KIT_SMSSERVICES_EVENTCODE_MSMS_BXEVENT")
			)*/
		);
		
		$event = new \Bitrix\Main\Event("kit.smsservices", "OnAfterEventsAdd",array("EVENTS"=>$events));
		$event->send();
		   if ($event->getResults()){
		...
		while($dt = $allType->fetch()){
			$events["MSMS_BXEVENT_".$dt['EVENT_NAME']] = array(
				...
				"NAME" => Loc::getMessage("KIT_SMSSERVICES_EVENTCODE_MSMS_BXEVENT").' - '.$dt['NAME']
			);
		}
	...
	public static function OnSaleOrderEntitySaved(\Bitrix\Main\Event $event){
		...
		if($order){
			...
			if($isNew){
				...
				$res = \Kit\Smsservices\EventlistTable::getList(
					array(
						'select' => array("*"),
						...
						foreach($phoneAr as $phone){
							...
							if(strlen($phone)>7){
								...
								if(trim($arData['TEMPLATE'])){
									$smsOb = new \Kit\Smsservices\Sender();
									$smsOb->event = $arMakros['#EVENT_NAME#'];
									$smsOb->eventName = Loc::getMessage("KIT_SMSSERVICES_EVENTCODE_MSMS_NEWORDER");
						...
						foreach($phoneAr as $phone){
							if(strlen($phone)>7){
								...
								if(trim($arData['TEMPLATE'])){
									$smsOb = new \Kit\Smsservices\Sender();
									$smsOb->event = $arMakros['#EVENT_NAME#'];
									$smsOb->eventName = Loc::getMessage("KIT_SMSSERVICES_EVENTCODE_MSMS_STATUSUPDATE");
									$smsOb->app = ($arData['PARAMS']['APPSMS']=='Y') ? true : false;
									$smsOb->sendSms($phone, $arData['TEMPLATE'],0,$sender);
						...
						foreach($phoneAr as $phone){
							if(strlen($phone)>7){
								...
								if(trim($arData['TEMPLATE'])){
									$smsOb = new \Kit\Smsservices\Sender();
									$smsOb->event = $arMakros['#EVENT_NAME#'];
									$smsOb->eventName = Loc::getMessage("KIT_SMSSERVICES_EVENTCODE_MSMS_PAYED");
									$smsOb->app = ($arData['PARAMS']['APPSMS']=='Y') ? true : false;
									$smsOb->sendSms($phone, $arData['TEMPLATE'],0,$sender);
	...
	public static function OnAdminTabControlBegin(&$form){
		...
		if( ($MODULE_RIGHT_ >= "R") && (($GLOBALS["APPLICATION"]->GetCurPage() == "/bitrix/admin/sale_order_view.php") || ($GLOBALS["APPLICATION"]->GetCurPage() == "/bitrix/admin/sale_order_edit.php")))
		{
			...
			if($orderId) {
				...
				$GLOBALS["APPLICATION"]->SetAdditionalCSS("/bitrix/css/kit.smsservices/style.css");
				
				$res = \Kit\Smsservices\ListTable::getList(array(
					'select' => array("*"),
					'filter' => array("=EVENT"=>'MSMS_ORDER_'.$orderId),
					'order' => array("TIME"=>"ASC")
				));
				$html = '<tr class="heading" id="tr_DETAIL_TEXT_LABEL">
					<td colspan="2">'.Loc::getMessage("KIT_SMSSERVICES_EVENTCODE_TABCONTROL_NAME").'</td>
				</tr>';
				...
				while ($arData = $res->fetch()){
					...
					<font class="status_'.(($arData['STATUS']==14 || $arData['STATUS']==15) ? 4 : $arData['STATUS']).'">'.Loc::getMessage("KIT_SMSSERVICES_LIST_STATUS_".$arData['STATUS']).'</font>
					</td>
				...
				$html .= '</table>
				<br/>
				<a href="/bitrix/admin/kit_smsservices_sendform.php?lang=ru&event=MSMS_ORDER_'.$orderId.'">'.Loc::getMessage("KIT_SMSSERVICES_EVENTCODE_SENDSMS").'</a>
				</td></tr>';
				
				$form->tabs[] = array("DIV" => "my_edit", "TAB" => Loc::getMessage("KIT_SMSSERVICES_EVENTCODE_TABCONTROL_NAME"), "ICON"=>"aszmagazin", "TITLE"=>Loc::getMessage("KIT_SMSSERVICES_EVENTCODE_TABCONTROL_NAME"), "CONTENT"=>$html);
					...
					foreach($phoneAr as $phone){
						if(strlen($phone)>7){
							...
							if(trim($arData['TEMPLATE'])){
								$smsOb = new \Kit\Smsservices\Sender();
								$smsOb->event = $arMakros['#EVENT_NAME#'];
								$smsOb->eventName = Loc::getMessage("KIT_SMSSERVICES_EVENTCODE_MSMS_BXEVENT");
								$smsOb->app = ($arData['PARAMS']['APPSMS']=='Y') ? true : false;
								$smsOb->sendSms($phone, $arData['TEMPLATE'],0,$sender);
