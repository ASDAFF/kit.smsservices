...
class Smsasistentby{
	...
	public function _sendSms ($phones, $mess, $time=0, $sender=false) {
		...
		$url =  'https://userarea.sms-assistent.by/api/v1/send_sms/plain?user='.$this->config->login.'&password='.$this->config->passw.'&recipient='.$phones.'&message='.$mess.'&sender='.$sender;
	...
	public function _getStatusSms($smsid,$phone=false) {
		...
		$url = 'https://userarea.sms-assistent.by/api/v1/statuses/plain?user='.$this->config->login.'&password='.$this->config->passw.'&id='.$smsid;
		$response = $this->openHttp($url);
	...
	public function _getBalance () {
		...
		$url = 'https://userarea.sms-assistent.by/api/v1/credits/plain?user='.$this->config->login.'&password='.$this->config->passw;
		$response = $this->openHttp($url);
