...
class Fields {
	...
	public static function newOrderHtml($value=""){
		...
		$macros = '';
		$MCR_EXT = self::getOrderCodes(Loc::getMessage("KIT_SMSSERVICES_FIELDS_MACROS_NEWORDER"));
		$html = '<tr><td>'.Loc::getMessage("KIT_SMSSERVICES_FIELDS_MACROS").'</td><td>'.$MCR_EXT.'</td></tr><tr>';
		$html .= '<td><b>'.Loc::getMessage("KIT_SMSSERVICES_FIELDS_TO").'</b></td>';
		$html .= '<td><input type="text" name="PARAMS_PHONE" value="'.$data['PHONE'].'"/></td>';
		$html .= '</tr>';
		
		$html .= '<td><b>'.Loc::getMessage("KIT_SMSSERVICES_FIELDS_APPSMS").'</b></td>';
	...
	public static function resetPasswordHtml($value=""){
		...
		$macros = '';
		$MCR_EXT = Loc::getMessage("KIT_SMSSERVICES_FIELDS_MACROS_RESET_PASSWORD");
		$html = '<tr><td>'.Loc::getMessage("KIT_SMSSERVICES_FIELDS_MACROS").'</td><td>'.$MCR_EXT.'</td></tr><tr>';
		$html .= '<td><b>'.Loc::getMessage("KIT_SMSSERVICES_FIELDS_TO").'</b></td>';
		$html .= '<td><input type="text" name="PARAMS_PHONE" value="'.$data['PHONE'].'"/></td>';
		$html .= '</tr>';
	...
	public static function payedOrderHtml($value=""){
		...
		$statusAr = array(array("NAME"=>Loc::getMessage("KIT_SMSSERVICES_FIELDS_PAYED_Y"),"ID"=>"Y"), array("NAME"=>Loc::getMessage("KIT_SMSSERVICES_FIELDS_PAYED_N"),"ID"=>"N"));
		
		$html .= '<td><b>'.Loc::getMessage("KIT_SMSSERVICES_FIELDS_PAYED").'</b></td>';
		$html .= '<td>';
	...
	public static function statusOrderHtml($value=""){
		...
		$statusAr = array(array("NAME"=>Loc::getMessage("KIT_SMSSERVICES_FIELDS_STATUS_ALL"),"ID"=>"ALL"));
		while($d = $statusOb->fetch()){
			$statusAr[$d['ID']] = $d;
		}
		
		$html .= '<td><b>'.Loc::getMessage("KIT_SMSSERVICES_FIELDS_STATUS_FROM").'</b></td>';
		$html .= '<td>';
		...
		$html .= '<td><b>'.Loc::getMessage("KIT_SMSSERVICES_FIELDS_STATUS_TO").'</b></td>';
		$html .= '<td>';
	...
	public static function eventSendHtml($value=""){
		...
		$defaultMactosText = "*".Loc::getMessage("KIT_SMSSERVICES_FIELDS_MACROS_BXEVENT");
		
		if(!$data['EVENT_NAME'] && $_REQUEST['EVENT']) {
			$data['EVENT_NAME'] = str_replace('MSMS_BXEVENT_','',$_REQUEST['EVENT']);
		}else{
			//print_r($_REQUEST['ID']);
			$dataAr = \Kit\Smsservices\EventlistTable::getRowById($_REQUEST['ID']);
		...
		if($data['EVENT_NAME']){
			...
			if(strpos($resType['DESCRIPTION'],'#ORDER_ID#')!==false && strpos($data['EVENT_NAME'],'SALE')!==false) {
				$defaultMactosText .= '<br>'.Loc::getMessage("KIT_SMSSERVICES_FIELDS_MACROS_NEWORDER_");
			}elseif(strpos($resType['DESCRIPTION'],'#USER_ID#')!==false) {
				$defaultMactosText .= '<br>'.Loc::getMessage("KIT_SMSSERVICES_FIELDS_MACROS_USER_");
			}
		}
		...
		$html = '<tr><td>'.Loc::getMessage("KIT_SMSSERVICES_FIELDS_MACROS").'</td><td>'.$defaultMactosText.'<br>*'.Loc::getMessage("KIT_SMSSERVICES_FIELDS_MACROS_BXEVENT_NOTE").'</td></tr><tr>';
		...
		$html .= '<td><b>'.Loc::getMessage("KIT_SMSSERVICES_FIELDS_EVENT_NAME").'</b></td>';
		$html .= '<td>';
		...
		$html .= '<td>'.Loc::getMessage("KIT_SMSSERVICES_FIELDS_BXEVENTID").'</td>';
		...
		$html .= '<td>'.Loc::getMessage("KIT_SMSSERVICES_FIELDS_BXEVENT_BREAK").'</td>';
		$checked = '';
	...
	public static function eventSendSave($arFields=array()){
		...
		if($PARAMS["EVENT_NAME"]){
			...
			if(!$exists) {
				...
				$id = $emess->Add(array('ACTIVE'=>"Y","EVENT_NAME"=>$PARAMS["EVENT_NAME"],"LID"=>array($_REQUEST['SITE_ID']),"EMAIL_FROM"=>"EMPTY@emptu.ru","EMAIL_TO"=>"EMPTY@emptu.ru","SUBJECT"=>"kit.smsservices","BODY_TYPE"=>"text","MESSAGE"=>"kit.smsservices"));
