...
$eventList = \Kit\Smsservices\Events::getList();
//print_r($eventList);
...
<?
$aTabs = array(
  array("DIV" => "edit1", "TAB" => Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_PARAM"), "ICON"=>"main_user_edit", "TITLE"=>Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_PARAM")),
);
...
if($REQUEST_METHOD == "POST" && ($save!="" || $apply!="") && $POST_RIGHT=="W" && check_bitrix_sessid()){
	...
	/*if(!$isUserHavePhpAccess)
	{
		...
		if($ID>0)
		{
			$emOldDb = \Kit\Smsservices\EventlistTable::getRowById($ID);
	...
	if($ID > 0){
		if(!$noupdate) {
			$res = \Kit\Smsservices\EventlistTable::update($ID,$arFields);
		}
	}else{
		if(!$noupdate) {
			$res = \Kit\Smsservices\EventlistTable::add($arFields);
		}
	}
	...
	if(!$noupdate){
		if(!$res->isSuccess() && count($errorAr)==0){
			...
			if ($_REQUEST['apply'] != "" && $ID>0){
				LocalRedirect("kit_smsservices_eventlist_edit.php?ID=".$ID."&mess=ok&lang=".LANG."&".$tabControl->ActiveTabParam());
			}
			elseif ($_REQUEST['apply'] != ""){
				LocalRedirect("kit_smsservices_eventlist_edit.php?ID=".$ID_new."&mess=ok&lang=".LANG."&".$tabControl->ActiveTabParam());
			}
			else{
				LocalRedirect("kit_smsservices_eventlist.php?lang=".LANG);
			}
...
if($ID>0)
{
	$dataAr = \Kit\Smsservices\EventlistTable::getRowById($ID);
	...
	if(is_array($dataAr)){
		...
		$errorAr[] = Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_ERROR_ID");
		$bVarsShowForm = false;
	}
...
?>
<?
$APPLICATION->SetTitle(($ID>0? Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_EDIT")." ID = ".$ID : Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_ADD")));
require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");


$aContext = array(
  array(
    "TEXT"=> Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_ADD_CERENCY"),
    "LINK"=> "kit_smsservices_eventlist_edit.php?lang=".LANG,
    "TITLE"=> Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_ADD_CERENCY"),
    "ICON"=> "btn_new",
  ),
);
...
if($_REQUEST["mess"] == "ok" && $ID>0)
  CAdminMessage::ShowMessage(array("MESSAGE"=>Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_SAVED"), "TYPE"=>"OK"));
...
<?if(!$str_EVENT){?>
<tr>
	<td width="40%"><?=Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_PARAM_EVENT")?></td>
	<td width="60%">
		<select id="EVENT" name="EVENT" onchange="this.form.submit();">
		<option value=""><?=Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_PARAM_EVENT_DEF")?></option>
...
<tr>
<td width="40%"><?=Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_PARAM_NAME")?></td>
<td width="60%">
...
<tr>
<td width="40%"><?=Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_PARAM_ACTIVE")?></td>
<td width="60%">
	<input type="checkbox" name="ACTIVE" value="Y"<?if($str_ACTIVE == "Y") echo " checked"?>/>
</td>
</tr>
<tr>
<td width="40%"><?=Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_PARAM_SITE_ID")?></td>
<td width="60%">
...
<tr>
<td width="40%"><?=Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_PARAM_SENDER")?></td>
<td width="60%">
	<input type="text" name="SENDER" value="<?=$str_SENDER?>"/>
	<br/><?=Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_PARAM_SENDER_DESC")?>
</td>
</tr>
<tr>
<td width="40%"><?=Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_PARAM_TEMPLATE")?></td>
<td width="60%">
	...
	<br/><?=Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_PARAM_TEMPLATE_DESC")?>
</td>
</tr>

<?if($eventDefault){
?>
<tr class="heading"><td colspan="2"><?=Loc::getMessage("KIT_SMSSERVICES_EVENTLIST_ADMIN_PARAM_PARAM_TITLE")?></td></tr>
<?
...
$tabControl->Buttons(
  array(
    ...
    "back_url"=>"kit_smsservices_eventlist.php?lang=".LANG,
