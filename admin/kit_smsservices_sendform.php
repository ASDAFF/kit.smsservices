...
?>
<?
$APPLICATION->SetTitle(Loc::getMessage("KITSS_SENDFORM_TITLE"));
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $MODULE_RIGHT == "W" && strlen($_REQUEST["Send"]) > 0)
{
	...
	if(!$_REQUEST['phone'] or !$_REQUEST['message']) {
		\ShowNote(Loc::getMessage("KITSS_SENDFORM_ERR_REQ"),'kiterror');
	}
	else{
		...
		if($phoneCheck['check']) {
			$resp = $smsServices->sendSms($phone, $message, MakeTimeStamp($datesend, "DD.MM.YYYY HH:MI:SS"),trim($_REQUEST['sender']),Loc::getMessage("KITSS_SENDFORM_PRIM"));
			if(!$resp->error){
				\ShowNote(Loc::getMessage("KITSS_SENDFORM_NOTICE_SEND"),'kitnoticeok');
				$phone = false;
				$message = false;
				if($orderId) LocalRedirect('/bitrix/admin/sale_order_view.php?ID='.preg_replace("/([^0-9])/is","",$_REQUEST['event']).'&lang='.LANGUAGE_ID);
			}else{
				\ShowNote(Loc::getMessage("KITSS_SENDFORM_ERR_".$resp->error_code).', ERROR: '.$resp->error,'kiterror');
			}
		}
		else{
			\ShowNote(Loc::getMessage("KITSS_SENDFORM_ERR_PHONE").', '.$phone,'kiterror');
		}
...
$aTabs = array(
	array("DIV" => "edit3", "TAB" => Loc::getMessage("KITSS_SENDFORM_TAB"), "ICON" => "vote_settings", "TITLE" => Loc::getMessage("KITSS_SENDFORM_TAB")),
);
...
?>
	...
	<?if($_REQUEST['event']){?>
	<tr><td colspan="2" style="text-align:center;font-weight:bold;"><?=Loc::getMessage("KITSS_SENDFORM_ORDER")?> <?=$_REQUEST['event']?><br/><br/></td></tr>
	<?}?>
	<?
	if($orderId){
		...
		if(count($phoneAr)>1){
			?>
			<tr><td colspan="2" style="text-align:center;font-weight:bold;"><?=Loc::getMessage("KITSS_SENDFORM_ORDERPHONE")?>: <?=implode(',',$phoneAr)?><br/><br/></td></tr>
			<?
		}
	...
	?>
	<tr>
		<td><?=Loc::getMessage("KITSS_SENDFORM_SENDER")?>:</td>
		<td>
			<?
	...
	<tr>
		<td><?=Loc::getMessage("KITSS_SENDFORM_PHONE")?>*:</td>
		<td>
	...
	<tr>
		<td><?=Loc::getMessage("KITSS_SENDFORM_MESS")?>*:</td>
		<td>
	...
	<tr>
		<td><?=Loc::getMessage("KITSS_SENDFORM_DATE")?>:</td>
		<td>
			<?echo \CalendarDate("datesend", $_REQUEST['datesend'], "datesend", "25", "class=\"date\"")?>
		</td>
	</tr>
	<tr>
		<td width="40%"><?=Loc::getMessage("KITSS_SENDFORM_TRANSLIT")?></td>
		<td width="60%">
			<input type="checkbox" name="translit" value="Y"/>
		</td>
	</tr>
	<tr>
		<td width="40%"><?=Loc::getMessage("KITSS_SENDFORM_APP")?></td>
		<td width="60%">
...
$tabControl->Buttons();
?>
	<input <?if ($MODULE_RIGHT<"W") echo "disabled" ?> type="submit" class="adm-btn-green" name="Sendform" value="<?=Loc::getMessage("KITSS_SENDFORM_SEND")?>" />
